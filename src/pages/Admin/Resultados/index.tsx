...
export const ResultadosPage = () => {
  ...
  const cyclesWithResults = useMemo(() => {
    const all = MOCK_CYCLES.filter(c =>
    ...
    return MOCK_PEOPLE.filter(person => {
      ...
      if (filters.buscarPersona) {
        ...
        if (
          ...
          !person.legajo.includes(q)
        )
          return false;
      }
      if (filters.evaluador && result?.evaluatorId !== filters.evaluador.value)
        return false;
      if (filters.proyecto && person.proyecto !== filters.proyecto.value)
        return false;
      if (filters.area && person.area !== filters.area.value) return false;
      if (
        filters.departamento &&
        person.departamento !== filters.departamento.value
      )
        return false;
      if (filters.provincia && person.provincia !== filters.provincia.value)
        return false;
  ...
  const completedCount = useMemo(() => {
    return filteredPeople.filter(p => {
      ...
      return (
        result &&
        Object.values(result.scores).some(score => score !== undefined)
      );
    }).length;
  }, [filteredPeople, selectedResults]);
    ...
    <DashboardLayout>
      <Stack sx={{ gap: 3 }}>
        ...
        <Stack sx={{ flexDirection: 'row', gap: 1, flexWrap: 'wrap' }}>
          {cyclesWithResults && cyclesWithResults.length > 0
            ...
            <Table sx={{ tableLayout: 'fixed', minWidth: 1200 }}>
              ...
              <TableBody>
                {filteredPeople.length === 0 ? (
                  <TableRow>
                    <TableCell
                      colSpan={SUB_DIMENSIONS.length + 1}
